...
export const winnticket: Project = {
  ...
  highlights: {
    ko: [
      ...
      'RBAC 설계 — 관리자/현장관리자/파트너 3단계 권한 분리, 라우트 가드 + API 가드 이중 보호',
      'Spring Boot + MyBatis + PostgreSQL RESTful API 50+ 엔드포인트 설계·구현',
      'JWT + Spring Security + Redis 세션 기반 인증/인가 시스템',
    ...
    en: [
      ...
      'RBAC design — 3-tier permission (admin/field/partner), route guard + API guard dual protection',
      'Spring Boot + MyBatis + PostgreSQL RESTful API 50+ endpoints designed and implemented',
      'JWT + Spring Security + Redis session-based auth/authorization system',
      'Multi-channel pricing engine — independent channel discounts, seasonal rates, partner discounts',
      'AWS S3 uploads, Excel data export, banner/popup scheduling',
    ]
  },
  techStack: {
